...
public class UserServiceImpl implements UserService {
    ...
    public PageEntry<User> selectPageUsers(PageEntry<User> pe) {
        int totalNum = userDao.getTotalPageNum(pe.getSelectCond());
        pe.setPageNum(totalNum % pe.getPageSize() == 0 ? totalNum / pe.getPageSize() : totalNum / pe.getPageSize() + 1);
        if (pe.getSelectCond() == null) {
            pe.setSelectCond(new User());
        }

        pe.setPagesInfo(userDao.getUserPage(pe));
        return pe;
    }
}
