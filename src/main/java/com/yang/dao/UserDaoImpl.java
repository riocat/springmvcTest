...
public class UserDaoImpl extends SqlSessionDaoSupport implements UserDaoMapper {
    ...
    @Override
    public int getTotalPageNum(User userCon) {
        return getSqlSession().getMapper(UserDaoMapper.class).getTotalPageNum(userCon);
    }

    @Override
    public List<User> getUserPage(PageEntry<User> pe) {
        return getSqlSession().getMapper(UserDaoMapper.class).getUserPage(pe);
    }
}
